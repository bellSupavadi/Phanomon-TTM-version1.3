...
function getData(){ // << สร้างฟังก์ชั่นขึ้นมาสักอันนึง
    ...
    axios.get('https://us-central1-phanomonttm.cloudfunctions.net/api/findDay/'+solaryear+'/'+solarmonth+'/'+solarday+'/',)
      .then(function (response) {
        console.log(response.data); // << ข้อมูลจะอยู่ใน response.data ให้เอาตัวแปรมาเก็บ 
    let objectMoon=response.data
    objectMoon.solaryear
    console.log(objectMoon.solaryear)
      })
      .catch(function (error) {
       console.log(error)
      });
